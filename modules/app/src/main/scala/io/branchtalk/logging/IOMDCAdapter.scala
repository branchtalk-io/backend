...
final class IOMDCAdapter(local: IOLocal[MDC.Ctx]) extends LogbackMDCAdapter {
  ...
  override def get(key:    String):                String = getMDC.get(key).orNull.asInstanceOf[String]
  ...
  def setContextMap(contextMap: ju.Map[?, ?]): Unit = setMDC(contextMap.asScala.toMap.asInstanceOf[MDC.Ctx])
